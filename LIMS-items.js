function getSKU() {
  var dept = document.getElementById('deptsku').value.substring(0,3);
  var firstVal = document.getElementById('inp1').value.substring(0,3); 
  var secondVal = document.getElementById('inp2').value.substring(0,3);
  var num1 = document.getElementById('num1').value.substring(0,4);
  var sku1 = dept+"-"+firstVal+"-"+secondVal+"-"+num1;
  var sku = sku1.toUpperCase();
	document.getElementById('output').value = sku;
...
var items = {
  ...
  list : async (toggle) => {
    ...
    hList.innerHTML = "";
    if (all.length==0) {
      hList.innerHTML = "<div class='row'><div class='grow'>No lost items found.</div></div>";
    } else { for (let i of all) {
      let row = document.createElement("div");
      row.className = "row flex";
      row.innerHTML =
      `<div class="grow">
      <br><div class="bold">[${i["sku"]}] ${i["name"]}</div><br>
      </div>
  ...
  // (B) DELETE ITEM
  del : async (sku) => { if (confirm("Confirm item deletion?")) {
  ...
  addEdit : async (sku) => {
    ...
    if (typeof sku == "string") {
      ...
      document.getElementById("output").value = item.sku;
  ...
  save : async () => {
    ...
    data = {
      sku : document.getElementById("output").value,
